...
public class WraptorResponse<T>(HttpStatusCode responseStatusCode, T? deserializeObject) where T: class
{
    ...
    {
        return new WraptorResponse<T>(response.StatusCode, response.IsSuccessStatusCode ? JsonConvert.DeserializeObject<T>(response.Content.ReadAsStringAsync().Result) : null);
    }
}